...
默认语言为 `en-US`，支持 `en-US` 和 `zh-CN`。按 `F1` → `Settings` 切换语言，选择会自动持久化。手动安装时，`KingdomEnhanced.dll` 同级必须包含 `Localization/en-US.json` 与 `Localization/zh-CN.json`。目录缺失或资源损坏时按英文回退；找不到对应文本时回退为资源键。
